...
path = 'C:/Users/cb/Downloads/GTN-master/gesture_990.any'
...
def train():
    ...
    for index in range(EPOCH):
        ...
        if ((index + 1) % test_interval) == 0:
            ...
            print(f'当前最大准确率\t测试集:{max(correct_on_test)}%\t 训练集:{max(correct_on_train)}% epoch = {max_index}')

        pbar.update()

    os.rename(f'{file_name} batch={BATCH_SIZE}.pkl',
              f'{file_name} {max_accuracy} {max_index} batch={BATCH_SIZE}.pkl')
